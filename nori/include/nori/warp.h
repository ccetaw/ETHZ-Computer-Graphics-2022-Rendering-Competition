...
public:
    ...
    /// Uniformly sample a vector on a Cylinder with radius 1 and low=z1, high=z2
    static Vector3f squareToUniformCylinder(const Point2f &sample, float z1, float z2);

    /// Probability density of \ref squareToUniformCylinder()
    static float squareToUniformCylinderPdf(const Vector3f &p, float z1, float z2);
