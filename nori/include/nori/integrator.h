...
public:
    ...
    /// Perform an (optional) postprocess step
    virtual void postprocess(int i) { 
        cout << "Image " << i  << " done\n";
        std::cout.flush();
    }

    /// If integrator will render multiple times
    virtual int multiple_render() {return 1;}

    /// Print progress utility function. Used when some process in the integrator is fairly long
    /// Progress indicator  https://ideone.com/Yg8NKj
