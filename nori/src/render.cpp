...
void RenderThread::renderScene(const std::string & filename) {
    ...
    if (root->getClassType() == NoriObject::EScene) {
        ...
        m_render_thread = std::thread([this,outputName] {
            ...
            auto numIterations = m_scene->getIntegrator()->multiple_render();

            // Make a directory for multiple outputs
            if (numIterations > 1)
            {
                std::string cmd = "mkdir " + outputName;
            ...
            for (int i=0; i < numIterations; i++)
            {
                ...
                for (uint32_t k = 0; k < numSamples ; ++k) {
                    m_progress = (k+1)/float(numSamples);
                ...
                m_scene->getIntegrator()->postprocess(i);
                ...
                if (numIterations == 1)
                    ...
                    bitmap->save(outputName + "/" + "output" + std::to_string(i) + ".exr");
